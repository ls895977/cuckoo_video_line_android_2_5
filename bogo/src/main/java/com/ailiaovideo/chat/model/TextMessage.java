...
import com.ailiaovideo.chat.utils.EmoticonUtil;
...
import com.ailiaovideo.videoline.modle.custommsg.CustomMsgText;
...
public class TextMessage extends Message {
    ...
    public TextMessage(Editable s) {
        ...
        message = customMsgText.parseToTIMMessage(message);
    }
    ...
    public void showMessage(final ChatAdapter.ViewHolder viewHolder, final Context context) {
        ...
        for (int i = 0; i < message.getElementCount(); ++i) {
            ...
            } else if (message.getElement(i).getType() == TIMElemType.Custom) {

                try {
                    String data = new String(((TIMCustomElem) message.getElement(i)).getData(), "UTF-8");
                    CustomMsg customMsg = JSON.parseObject(data, CustomMsg.class);

                    setSenderUserInfo(viewHolder, context, customMsg.getSender());

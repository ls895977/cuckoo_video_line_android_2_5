...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) MyApplication.getInstances().getSystemService(MyApplication.getContext().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(MyApplication.getInstances());
        Intent notificationIntent = new Intent(MyApplication.getInstances(), HomeActivity.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(MyApplication.getInstances(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager)MyApplication.getInstances().getSystemService(MyApplication.getInstances().NOTIFICATION_SERVICE);
